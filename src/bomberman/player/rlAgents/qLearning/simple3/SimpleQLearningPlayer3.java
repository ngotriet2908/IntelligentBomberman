...
public class SimpleQLearningPlayer3 extends RLPlayer {
    ...
    private static final int REWARD_BOMB = -5;
    ...
    private static final int REWARD_KILL = +200;
...
//        simpleState.setPlacedBombs(getMinStepToOpponent(game));
        ...
        game.getBombs().sort(Comparator.comparing(bomb -> bomb.getTile().getCoordinate().x*1000 + bomb.getTile().getCoordinate().y));
...
//        );
    ...
    private int rewardFunction(int tileNum) {
        return Math.max((int)(5.0*REWARD_BOMB/(1 + tileNum)), -25);
    }

    private int rewardFunctionMove(int tickCount) {
        return Math.max((int)(REWARD_MOVE*tickCount/10) , -20);
    }

    private int rewardFunctionInvalidMove(int tickCount) {
        return Math.max((int)(REWARD_MOVE*tickCount/10) , -20);
    }
